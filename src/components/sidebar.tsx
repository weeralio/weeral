...
export default function Sidebar({ email }: { email: string }) {
  ...
  return (
    <aside className="w-60 shrink-0 h-screen sticky top-0 bg-[#0a0a18] border-r border-[#1e1e3f] flex flex-col">
      {/* Logo */}
      <div className="h-14 flex items-center px-5 border-b border-[#1e1e3f]">
          ...
          return (
            <Link
              ...
              className={`relative flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all duration-150 group ${
                isActive
                  ? 'text-white'
                  : 'text-[#475569] hover:text-[#94a3b8] hover:bg-[#111128]'
              }`}
            >
              {isActive && (
                <motion.div
      ...
      {/* User */}
      <div className="px-3 py-4 border-t border-[#1e1e3f]">
        <div className="flex items-center gap-2.5 px-3 mb-1.5">
          <div className="w-7 h-7 rounded-full bg-gradient-to-br from-violet-600 to-purple-700 flex items-center justify-center text-xs font-bold text-white shrink-0 shadow-[0_0_8px_rgba(139,92,246,0.3)]">
            {email[0].toUpperCase()}
          </div>
        ...
        <form action={logout}>
          <button
            type="submit"
            className="flex items-center gap-3 px-3 py-2 w-full rounded-xl text-sm text-[#475569] hover:text-[#94a3b8] hover:bg-[#111128] transition-all group"
          >
            <svg className="w-4 h-4 group-hover:text-[#8b5cf6]/60 transition-colors shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
